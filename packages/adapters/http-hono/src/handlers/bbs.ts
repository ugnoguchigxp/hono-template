...
export const createListThreadsHandler =
  (useCase: ListThreadsUseCase): Handler =>
  async (c) => {
    const threads = await useCase.execute();
    return c.json({ data: threads });
  };

export const createThreadDetailHandler =
  (useCase: GetThreadDetailUseCase): Handler =>
  async (c) => {
...
export const createCreateThreadHandler =
  (useCase: CreateThreadUseCase): Handler =>
  async (c) => {
...
export const createPostCommentHandler =
  (useCase: PostCommentUseCase): Handler =>
  async (c) => {
    const user = c.get('user');
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
